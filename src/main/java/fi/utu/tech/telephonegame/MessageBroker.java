...
public class MessageBroker extends Thread {
	...
	private Message process(Object procMessage) {
		if (!(procMessage instanceof Message)) {
			System.err.println("Error: processed message isn't the Message class.");
		}

		Message message = (Message) procMessage;
		prevMessages.put(message.getUUID());

		gui_io.setReceivedMessage(message.getMessage());

		//tallentuuko refinetetty viesti samaan viestiin? nyt niin käy...
		message.setMessage(Refiner.refineText(message.getMessage()));
		message.setColor(Refiner.refineColor(message.getColor()));

		gui_io.setSignal(message.getColor());

		gui_io.setRefinedMessage(message.getMessage());
